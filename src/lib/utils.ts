 ...
 * returning a maximum of 10 numbers, with missing ranges replaced with ellipses
 ...
 *
 * @param {number} totalResults (Required: The total number of results to
 *    expect, the end number.)
 *
 * @param {number} startNumber (Required: The value/start number of the page
 *    currently displayed.)
 *
 * @param {number} resultsPage (Optional: The number of results to show for
...
export function paginate(
  ...
  const allNumbers = [totalResults, resultsPage, startNumber].every(
    Number.isFinite,
  );
  ...
  if (!allNumbers) {
    throw new TypeError(
      "pagination() expects only finite numbers as arguments.",
    );
  }
  ...
  if (startNumber < 0) {
    throw new Error(
      "start_number must be a number greater than or equal to 0.",
    );
  }

  // Determine the length of the array to be returned, which is the total
  // results divided by the number of results per page.
  const length = Math.ceil(Math.abs(totalResults / resultsPage));

  // 10 pages or less, just return the array
  if (length <= 10) {
    // Fill up a new array with the range numbers
    // using Array.from() with a mapping function.
    const full_array = Array.from(Array(length), (x, index) => index + 1);
    return full_array;
  }

  let current_page = startNumber / resultsPage + 1;
  if (startNumber == 1) {
    current_page = 1;
  }
...
export const isStaticExport = import.meta.env.VITE_SITE_MODE === "static";
